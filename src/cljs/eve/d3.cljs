...
;; (defn foo []
;;   (let [w (.style (.select d3 ".trend-card") "width")
;;         h (.style (.select d3 ".trend-card") "height")
;;         local-svg (-> d3
;;                       (.select ".trend-card")
;;                       (.append "svg")
;;                       (.attr "width" w)
...
(defn test-chart []
  ((.-csv d3)
   ...
   (fn [error data]
     ...
     (.domain y (.extent d3 data (fn [d] (.-close d))))
...
(def x (.range (.linear (.-scale d3)) (array 0 width)))
(def y (.range (.linear (.-scale d3)) (array height 0)))
...
(def x-axis (-> (.-svg d3) (.axis) (.scale x) (.orient "bottom")))
(def y-axis (-> (.-svg d3) (.axis) (.scale y) (.orient "left")))

(def line (-> (.line (.-svg d3))
              (.x (x (fn [[x _]] x)))
              (.y (y (fn [[_ y]] y)))))

(def data (partition 2 (interleave
                        (take 40 (iterate (fn [_] (rand 1)) (rand 1)))
                        (take 40 (iterate (fn [_] (rand 1)) (rand 1))))))
...
;;      (-> focus
;;          (.append "path")
;;          ;; You use datum when you want to clear previously seleced data - I do
;;          ;; belive that this is the case here.
;;          (.datum data)
;;          (.attr "clip-path" "url(#clip)")
;;          (.attr "d" area))
